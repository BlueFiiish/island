...
(function (root) {
  ...
  // The Command Center's HQ API answers on the tailnet host over https.
  var DEFAULT_BASE = 'https://bigbertha.taild5eb1b.ts.net:8443';

  // The Command Center's own web UI, which is a DIFFERENT port since the
  // 2026-09-09 hardening: :8443 is now scoped to `/api/hq` at the Tailscale
  // serve layer (everything else there 404s before it reaches the app), and the
  // full UI/PWA moved to :8447. Only the "Open the Command Center" button in
  // the settings sheet uses this - it is a link the user taps, never a fetch,
  // and no token is attached to it.
  var UI_BASE = 'https://bigbertha.taild5eb1b.ts.net:8447';

  /**
   * A-01 / C-01: THE ONLY ORIGINS THIS ROUTE MAY EVER TALK TO.
   *
   * Until 2026-09-09 the base was freely overridable at runtime - the deep
   * link's `&b=` and the settings-sheet field both wrote whatever string they
   * were handed straight into localStorage, and every subsequent call sent
   * `Authorization: Bearer <the real Command Center token>` to it. One link
   * (`/island/hq/#b=hq.attacker.example`) handed Josia's PC credential to
   * anyone who could get that URL in front of his phone, with no prompt: the
   * hash is stripped by replaceState, so the address bar looked clean.
   *
   * So the base is now COMPILED IN, not configurable. `b=` is accepted only
   * when it names an origin on this list; anything else is ignored and the
   * stored base is reset to the default. The check is repeated INSIDE the
   * fetch wrapper (see call() below) so a token can never ride to an
   * off-list origin even if some future path writes storage directly.
   *
   * Adding an entry here is a deliberate, reviewed edit - a rename or a port
   * change is a redeploy now, which is the correct price for the token.
   */
  var ALLOWED_BASES = [DEFAULT_BASE];

  /**
   * The normalised form of `v` when it is one of ALLOWED_BASES, else ''.
   * Compared origin-wise (scheme + host + port), so a trailing slash, a case
   * difference in the host or an appended path cannot smuggle a mismatch past
   * a plain string compare - and a path suffix is refused outright, because
   * the client concatenates `base + '/api/hq/...'`.
   */
  function allowedBase(v) {
    var s = normBase(v);
    if (!s) return '';
    var u;
    try {
    ...
    var got = u.protocol + '//' + u.host;
    for (var i = 0; i < ALLOWED_BASES.length; i++) {
      var want;
      try {
        var a = new URL(ALLOWED_BASES[i]);
        want = a.protocol + '//' + a.host;
      } catch (e2) {
        continue;
      }
      if (got.toLowerCase() === want.toLowerCase()) return ALLOWED_BASES[i];
    }
    return '';
  }
  ...
  function create(cfg) {
    var conf = cfg || {};
    // A-01: the base is CLAMPED to the allowlist here, not merely validated at
    // the place it was typed. Whatever storage holds, an off-list value can
    // never become this client's base - it silently falls back to the default.
    var base = allowedBase(conf.base) || DEFAULT_BASE;
    var token = String(conf.token == null ? '' : conf.token).trim();
    if (conf.mock) return mockClient(base, token);

    function call(method, path, body) {
      // A-01, second gate: re-assert on EVERY call, immediately before the
      // bearer token is attached. `base` is a closure variable and nothing
      // reassigns it, so this can only fire if the allowlist itself was
      // tampered with at runtime - which is exactly the case worth refusing.
      if (allowedBase(base) !== base) {
        return Promise.resolve(
          envelope(false, 0, null, 'refused: the Command Center address is not allowlisted', { offline: true })
        );
      }
      var init = {
        method: method,
      ...
      return fetch(base + path, init).then(
        function (res) {
          if (res.status === 401 || res.status === 403) {
            // C-06: the body is READ, not discarded. The Command Center's 401
            // carries a machine-readable reason (Lane B, 2026-09-09) so the UI
            // can say "the key expired" instead of the generic "rejected" - and
            // an older server that sends no body still lands on the generic
            // copy, because every field below is optional.
            return res
              .json()
              .catch(function () {
                return null;
              ...
              .then(function (doc) {
                return envelope(false, res.status, doc, (doc && (doc.error || doc.detail)) || 'token rejected', {
                  unauthorized: true,
                });
              });
          }
  ...
  function client(base, call, isMock) {
    return {
       ...
       *
       * C-07 (2026-09-09): "an absolute http(s) URL" used to mean ANY host.
       * The scheme check did block `javascript:`/`data:`, so this was never an
       * XSS - but a compromised (or attacker-chosen, see A-01) Command Center
       * could point every <video src>, <img src> and <a href> on the page at
       * its own infrastructure: an IP + User-Agent + timing leak on load and a
       * plausible phishing target on tap. R14 already guarantees every peek and
       * thumb URL is signed and served BY the Command Center, so nothing
       * legitimate lives off `base`. Absolute URLs must now sit under it.
       */
      mediaUrl: function (p) {
        if (!p) return null;
        var s = String(p);
        if (/^https?:\/\//i.test(s)) return s === base || s.indexOf(base + '/') === 0 ? s : null;
