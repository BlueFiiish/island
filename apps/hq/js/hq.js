...
(function () {
  ...
  // C-06: the four storage keys are NAMESPACED TO THE DEPLOY BASE. hq.astro
  // computes them with lib/base.ts `hqStorageKey()` and hands them over on
  // window.FI_HQ.keys, so this file and RouteRail.astro can never drift apart.
  // The literals below are the fallback for a payload loaded outside the Astro
  // page (the mount source opened directly, a unit harness).
  var KEYS = CFG.keys || {};
  var TOKEN_KEY = KEYS.token || 'island.hq.token';
  var BASE_KEY = KEYS.base || 'island.hq.base';
  var MOCK_KEY = KEYS.mock || 'island.hq.mock';
  var TAB_KEY = KEYS.tab || 'island.hq.tab';

  // The un-namespaced keys this route shipped with until 2026-09-09. A phone
  // that is already linked must stay linked, so they are migrated forward once
  // and then removed - not read on every access, which would re-open the very
  // collision the namespace closes.
  var LEGACY_KEYS = {
    token: 'island.hq.token',
    base: 'island.hq.base',
    mock: 'island.hq.mock',
    tab: 'island.hq.tab',
  };
  ...
  /**
   * One-shot migration of the pre-namespace keys, plus the A-01 storage sweep.
   *
   * Runs before anything reads storage. Two jobs:
   *   1. move each legacy key's value to its namespaced name and delete the old
   *      one, so an already-linked phone survives this change;
   *   2. HARD-RESET a stored base that is not on the compiled-in allowlist. A
   *      device that was pointed at an attacker's host by the old `#b=` bug
   *      (C-01) is repaired by loading the page once - it does not need Josia
   *      to notice and re-link.
   */
  function migrateStorage() {
    var names = ['token', 'base', 'mock', 'tab'];
    for (var i = 0; i < names.length; i++) {
      var legacy = LEGACY_KEYS[names[i]];
      var current = KEYS[names[i]] || legacy;
      if (current === legacy) continue;
      var v = get(legacy);
      if (v != null && get(current) == null) set(current, v);
      if (v != null) del(legacy);
    }
    var stored = get(BASE_KEY);
    if (stored && !API.allowedBase(stored)) del(BASE_KEY);
  }
  ...
  // ------------------------------------------------------------------ state
  // Namespace migration + the allowlist sweep run BEFORE the first read.
  migrateStorage();

  var S = {
    baseRefused: false, // A-01: a deep link tried to repoint the API base
   ...
   *
   * A-01 / C-01 (2026-09-09). `b=` USED TO BE STORED VERBATIM. Every later call
   * then sent `Authorization: Bearer <the real Command Center token>` to
   * whatever host the link named, and because `b=` was honoured on its own the
   * attacker never needed to know the token - one URL in a DM was the whole
   * exploit. Now `b=` is accepted ONLY when it names an origin on
   * API.ALLOWED_BASES; anything else is ignored, the stored base is reset to
   * the default, and the status line says so. hq-api.js repeats the check at
   * the fetch wrapper, so parse-time is not the only place it is enforced.
   */
  function consumeHash() {
    ...
    if (base) {
      var ok = API.allowedBase(base);
      if (ok) {
        set(BASE_KEY, ok);
      } else {
        // Refuse AND repair: a phone that arrived here from a hostile link is
        // put back on the compiled-in default rather than left on whatever it
  ...
  /** The one place a base is chosen. Off-allowlist storage never wins. */
  function currentBase() {
    return API.allowedBase(get(BASE_KEY)) || API.DEFAULT_BASE;
  }
  ...
  function showUnlock(msg) {
    ...
    if (S.baseRefused) {
      text =
        'That link tried to point this phone at a different address, so it was ignored. ' +
        'HQ only ever talks to the Command Center.' + (text ? ' ' + text : '');
    }
    st.className = text ? 'hq-status bad' : 'hq-status';
    st.textContent = text;
  }
  ...
  /**
   * The Command Center refused this phone's key (401/403).
   *
   * ONE handler for every call site, because the wrong shape here is a phone
   * that quietly re-sends a dead credential forever. It: stops the loop, wipes
   * the stored token (never the base - that is compiled in), and shows the
   * re-link screen with the server's own reason when it sent one.
   *
   * THE WIRE SHAPE, from the Command Center (Lane B, 2026-09-09). Both are
   * HTTP 401 with a JSON body; the client branches on `code`:
   *
   *   { "error": "hq token expired", "code": "hq_token_expired",
   *     "expired_at": "<local ISO-8601, display only>", "action": "remint" }
   *   { "error": "auth required",    "code": "hq_auth_required" }
   *
   * HQ tokens now carry a 30-day TTL, so "expired" is the COMMON case and has
   * to read as routine maintenance, not as a break-in. Every field is treated
   * as optional: an older Command Center sends no body at all and still lands
   * on the generic line, and unknown server text is never printed at the user.
   */
  var REJECT_COPY = {
    hq_token_expired: 'This phone’s key has expired.',
    hq_auth_required: 'The Command Center did not recognise this phone’s key.',
    // Kept for a future revoke code / an older server that sent a bare reason.
    expired: 'This phone’s key has expired.',
    revoked: 'This phone’s key was revoked on the PC.',
  };
  function onTokenRejected(res) {
    stopPolling();
    del(TOKEN_KEY);
    S.client = null;
    S.status = null;
    var doc = (res && res.data) || {};
    var code = String(doc.code || doc.reason || '').toLowerCase();
    var lead = REJECT_COPY[code] || 'The Command Center rejected this phone’s key.';
    // `expired_at` is display-only text from the server, so it goes through
    // showUnlock -> textContent like everything else on this route, and only
    // when it is a short, plausible timestamp.
    var when = String(doc.expired_at || '');
    if (code === 'hq_token_expired' && /^[0-9T:\-. ]{8,32}$/.test(when)) {
      lead += ' (' + when.replace('T', ' ') + ')';
    }
    showUnlock(lead + ' Scan the QR on the Command Center home page to link it again.');
  }
  ...
  function tick() {
    ...
    S.client.status().then(function (res) {
      ...
      if (res.unauthorized) {
        // C-06 (2026-09-09): a rejected key used to be kept in localStorage and
        // re-sent on a 20s loop forever - a dead credential sitting on a shared
        // origin (C-04) with nothing to gain by staying there. Now the token is
        // DROPPED on the first 401/403 and the route drops to its re-link
        // state. Nothing is auto-retried: only a fresh key from the PC helps.
        onTokenRejected(res);
        return;
      }
      if (res.ok) {
  ...
  function openSettingsSheet() {
    openSheet('Setup', function (body) {
      ...
      // The API base is scoped to /api/hq at the serve layer since
      // 2026-09-09, so linking at it would 404. The UI lives on :8447.
      open.href = API.UI_BASE;
      open.target = '_blank';
      open.rel = 'noopener';
      ...
      baseIn.value = currentBase();
      baseField.appendChild(baseIn);
      body.appendChild(baseField);
      // A-01: the address is compiled in. Say so, rather than letting the field
      // look like a free-text setting that simply refuses to stick.
      body.appendChild(
        e(
          'p',
          'hq-status',
          'Fixed to the Command Center. A different address is refused - changing it is a redeploy.'
        )
      );
      ...
      saveBase.addEventListener('click', function () {
        var ok = API.allowedBase(baseIn.value);
        if (!ok) {
          baseIn.value = currentBase();
          toast('That address is not the Command Center - ignored');
          return;
        }
        set(BASE_KEY, ok);
        makeClient();
  ...
  function doAssign(rec, gameKey) {
    ...
    S.client.assign(rec.id, gameKey).then(function (r) {
      if (!r.ok) {
        rec.state = was;
        // C-06: a 401 on an action gets the SAME treatment as a 401 on the
        // poll - drop the key and re-link. Anything else is a normal failure.
        if (r.unauthorized) {
          renderPanel();
          onTokenRejected(r);
          return;
  ...
  function boot() {
    ...
    $('hq-unlock-go').addEventListener('click', function () {
      ...
      var typedBase = API.allowedBase($('hq-base').value);
      if (!typedBase) {
        // A-01: refuse the address, keep the key. The default is the only
        // place the token is ever allowed to go.
        $('hq-base').value = API.DEFAULT_BASE;
        del(BASE_KEY);
        var st2 = $('hq-unlock-status');
        st2.className = 'hq-status bad';
        st2.textContent = 'That is not the Command Center address - using the built-in one.';
      } else {
        set(BASE_KEY, typedBase);
      }
      set(TOKEN_KEY, tok);
      $('hq-token').value = '';
      makeClient();
    ...
    if (get(TOKEN_KEY)) {
      showApp();
      // A-01: an already-linked phone that arrived from a hostile deep link
      // gets told, rather than silently continuing on the (correct) default.
      if (S.baseRefused) toast('A link tried to change the Command Center address - ignored');
    } else showUnlock('');
  }
