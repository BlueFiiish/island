...
(function () {
  ...
  /* The ONE beginner-section renderer lives in app.js (DBD.beginnerHtml) so
     every entity detail surface in the route renders the hand-authored
     What it is / Why it matters / When to use it block identically, and
     renders NOTHING when a record carries no prose. Null-safe: a core that
     has not landed yet degrades to no block rather than throwing. */
  function beginnerHtml(e) {
    var f = window.DBD && window.DBD.beginnerHtml;
    try { return typeof f === 'function' ? f(e) : ''; } catch (err) { return ''; }
  }
  ...
  /* The character screen, both branches. validate.mjs group N slices this whole
     block out between the sentinels below and EXECUTES it against a real killer
     record and a real survivor record with the real DBD.beginnerHtml wired in,
     so "the beginner block renders" is proven on EVERY branch rather than on
     whichever branch a reader happened to check. The killer branch shipped with
     no character-level block at all because the only gate looked for a call
     inside loreCardHtml, which killers never reach. Do not remove the sentinels,
     and do not add a third branch without adding it to the gate's branch list.
     Everything between the sentinels must stay callable with only the stubs the
     gate supplies: esc, art, chapterLine, norm, beginnerHtml and the three
     section renderers. */
  /* GATE:characterScreen:start */
  function loadoutHtml(c) {
    ...
    return hero +
      (isK ? powerCardHtml(c) + killerBeginnerHtml(c) : loreCardHtml(c)) +
      perkSectionHtml(c) +
      gearSectionHtml(c) +
      teachSectionHtml(c);
  }
  ...
  function loreCardHtml(c) {
    var beg = beginnerHtml(c);
    if (!c.lore && !beg) return '';
    return '<section class="dbdl-sec">' + beg +
      (c.lore ? '<p class="dbdl-lore">' + esc(c.lore) + '</p>' : '') + '</section>';
  }

  /* The KILLER branch's character-level beginner block, rendered directly under
     the power card. It is deliberately NOT merged into powerCardHtml and not
     merged with the power's own block on the power detail sheet: that prose is
     about the POWER, this prose is about picking the KILLER. Same renderer, same
     .beg classes, no extra CSS - it just needs a .dbdl-sec to sit in so it keeps
     the screen's normal section spacing. Empty in, empty out. */
  function killerBeginnerHtml(c) {
    var beg = beginnerHtml(c);
    return beg ? '<section class="dbdl-sec">' + beg + '</section>' : '';
  }
  /* GATE:characterScreen:end */
