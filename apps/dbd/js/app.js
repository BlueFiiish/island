...
(function (DBD) {
  ...
  // ===========================================================================
  // Beginner sections — the hand-authored layer (NEW-GAME-TEMPLATE 7D).
  //
  // The prose lives in tools/beginner-notes/<group>.json and is merged FORWARD
  // into every data record by build_data.mjs as `entry.beginner`, so this is
  // the ONE renderer for it and every entity detail surface in the route calls
  // it — the wiki sheets below, the loadout detail sheets, the shrine perk
  // sheet, and the character screen.
  //
  // Present → the three fixed sections, in the contract's order, with those
  // exact headings. Absent → the EMPTY STRING, so a record with no prose shows
  // no heading, no placeholder and no gap. Partial → only the sections that
  // were actually written; a heading over nothing is worse than no heading.
  //
  // The prose is PLAIN TEXT and is escaped, never run through formatDesc():
  // formatDesc re-allows a tag whitelist for the source's own HTML fragments,
  // and this text has no HTML to re-allow.
  //
  // The sentinels are load-bearing: validate.mjs group N slices this function
  // out of the file and executes it against the real built data, so the gate
  // proves what the route renders rather than trusting that it does. Do not
  // remove them.
  /* GATE:beginnerHtml:start */
  DBD.BEGINNER_SECTIONS = [
    ['whatItIs', 'What it is'],
    ['whyItMatters', 'Why it matters'],
    ['whenToUseIt', 'When to use it']
  ];
  DBD.beginnerHtml = function beginnerHtml(entry) {
    const b = entry && entry.beginner;
    if (!b) return '';
    const parts = DBD.BEGINNER_SECTIONS
      .filter(function (s) { return typeof b[s[0]] === 'string' && b[s[0]].trim(); })
      .map(function (s) {
        return '<div class="beg-h">' + DBD.esc(s[1]) + '</div>' +
          '<p class="beg-p">' + DBD.esc(b[s[0]].trim()) + '</p>';
      });
    if (!parts.length) return '';
    return '<div class="beg">' + parts.join('') + '</div>';
  };
  /* GATE:beginnerHtml:end */
