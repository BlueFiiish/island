...
/* R-01 (2026-09-09, repo audit). A literal Tailscale CGNAT address
   used to be hardcoded here, and it shipped to the PUBLIC site
   at /island/apps/isaac/app.js. It is not routable off the tailnet, so it was
   never direct access - but it named the exact node AND port of a private
   service on Josia's gaming PC to anyone who read the site's JS, and it is a
   standing R11 violation the personal-data gate could not see (it had no IP
   pattern; it does now). No address is baked in any more: the phone/other-device
   link is opt-in, set locally by whoever runs the server, and when it is unset
   the card explains the shape of the URL instead of publishing one. */
const LIVEMAP_LAN = () => window.ISAAC_LIVEMAP_LAN || "";
...
function renderLiveMap(){
  ...
  setup.innerHTML=`<h2>Live map is not running</h2>
    ...
    <p class="dim">On a phone or another device, open the map directly:
      ${LIVEMAP_LAN()
        ? `<a href="${esc(LIVEMAP_LAN())}" rel="noopener">${esc(LIVEMAP_LAN())}</a> (needs Tailscale).`
        : `<code>http://&lt;the gaming PC&gt;:8851/</code> &mdash; its Tailscale name or LAN address, on the same tailnet.`}
      <b>Safari</b> cannot reach a local server from this
