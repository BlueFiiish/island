...
(function (root, factory) {
  ...
  function render(mount, ctx, param) {
    ...
    return {
      ...
      // Quest/profile pushes (Refresh from Pilot) must NOT tear the open
      // interactive map down - it redraws itself via its own live "quests"
      // case. renderDetail only clears the right pane; imapMount is a sibling
      // in wrap, so repainting underneath the map is safe.
      refresh: () => {
        paintCards();
