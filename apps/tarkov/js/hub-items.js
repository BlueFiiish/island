...
(function (root, factory) {
  ...
  function render(mount, ctx, param) {
    ...
    function renderDetail(id) {
      ...
      // guns and factory presets open in the weapon builder (a preset lands
      // fully assembled; a bare gun lands with its slots empty)
      // Only a gun with slots, or a preset whose base is one, can open there:
      // armour/helmet presets and slotless "guns" (flares) would dead-end.
      const itTypes = Array.isArray(it.types) ? it.types : [];
      const builderParam = itTypes.indexOf('gun') >= 0 ? 'gun/' + id : (itTypes.indexOf('preset') >= 0 ? 'preset/' + id : '');
      if (builderParam && ctx.go) {
        ensureProps().then((all) => {
          if (state.selected !== id || !all) return;
          // helmets and head mounts carry slots too, so "has slots" alone
          // would still send 6 helmet presets to a blank builder
          const isGun = (pid) => { const g = items[pid]; return !!(g && Array.isArray(g.types) && g.types.indexOf('gun') >= 0); };
          const slotted = (pid) => { const p = all[pid]; return isGun(pid) && !!(p && Array.isArray(p.slots) && p.slots.length); };
          const pr = all[id];
          const can = itTypes.indexOf('gun') >= 0 ? slotted(id) : !!(pr && pr.baseItem && slotted(pr.baseItem));
          if (!can) return;
          const ob = el('button', 'wiki-btn', 'Open in Builder');
          ob.type = 'button';
          ob.addEventListener('click', () => ctx.go('builder', builderParam));
          headText.appendChild(ob);
        }, () => { /* props unavailable: no builder button */ });
      }
