...
(function (root, factory) {
  ...
  function render(mount, ctx, param) {
    ...
    function paintProfile() {
      ...
      // the builder is the other half of "what do I take into raid": build the
      // gun slot by slot, then kit around it
      const wb = el('button', 'kit-link', 'Open the weapon builder');
      wb.type = 'button';
      wb.addEventListener('click', () => ctx.go('builder', ''));
      profileLine.appendChild(el('span', 'muted', ' - '));
      profileLine.appendChild(wb);
    }
