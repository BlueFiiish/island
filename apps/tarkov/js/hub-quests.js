...
(function (root, factory) {
  ...
  function render(mount, ctx, param) {
    ...
    // ---- "Sync quests" row ----
    // Host-conditional, exactly like setQuestStatus below: the Electron hub
    // provides ctx.syncQuestsNow (a full re-read of the game's own log
    // sessions), the web host does not - its refresh is the site's own
    // "Refresh from Pilot" button, so nothing is drawn here for it.
    //
    // The readout next to the button is the whole point. "The list is empty"
    // has three causes a player cannot tell apart (wrong Logs folder, no
    // sessions, no quest events yet), and main names which one it was.
    let syncBtn = null;
    let syncMsg = null;
    if (typeof ctx.syncQuestsNow === 'function') {
      const syncRow = el('div', 'quest-sync');
      syncBtn = el('button', 'chip quest-sync-btn', 'Sync quests');
      syncBtn.type = 'button';
      syncBtn.title = 'Re-read every Tarkov log session for quest progress';
      syncBtn.addEventListener('click', () => { ctx.syncQuestsNow(); });
      syncMsg = el('span', 'items-count quest-sync-msg');
      syncRow.appendChild(syncBtn);
      syncRow.appendChild(syncMsg);
      head.appendChild(syncRow);
    }
    function syncRowText(r) {
      if (!r || typeof r !== 'object') return 'Not synced yet';
      if (r.state === 'running') return 'Syncing quests...';
      const base = r.text || r.message || (r.state === 'done' ? 'Synced' : 'Not synced yet');
      if (r.state === 'done' && Number.isFinite(r.at) && typeof ctx.ago === 'function') {
        return `${base} (${ctx.ago(r.at)})`;
      }
      return base;
    }
    function renderSyncRow() {
      if (!syncBtn) return;
      const r = ctx.questSync || { state: 'idle' };
      const running = r.state === 'running';
      syncBtn.disabled = running;
      syncBtn.textContent = running ? 'Syncing...' : 'Sync quests';
      syncMsg.textContent = syncRowText(r);
      // Only a FINISHED report can be bad: a running one still carries the
      // previous result's folder flags.
      syncMsg.classList.toggle('bad', r.state === 'error'
        || (r.state === 'done' && (r.logsDirExists === false || (r.sessionsTotal | 0) === 0)));
    }
    ...
    return {
      ...
      // a quest event, a profile save or a sync report moved every badge
      refresh: () => { recompute(); renderDetail(state.selected); renderSyncRow(); },
    };
  }
