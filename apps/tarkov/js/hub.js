...
(function (root, factory) {
  ...
  const ROUTES = [
    ...
    { id: 'builder', label: 'Builder', ready: true, view: 'PilotHubBuilder' },
  ];
  ...
  function hashFor(route, param) {
    ...
    if (!param) return '#/' + r;
    // encode each segment on its own so a param like 'saved/<id>' keeps its
    // literal '/' in the visible hash instead of becoming 'saved%2F<id>';
    // parseHash still accepts the old whole-string-encoded form too.
    const segs = String(param).split('/').map((s) => encodeURIComponent(s));
    return '#/' + r + '/' + segs.join('/');
  }
  ...
  function start() {
    ...
    // builder.json = factory preset part lists + mutually-exclusive mod
    // pairs. Optional: the builder view tolerates null (no presets, no
    // conflict greying) rather than refusing to render.
    function ensureBuilder() {
      if (ctx.builderData !== undefined) return Promise.resolve(ctx.builderData);
      return loadJson('wiki/builder.json').then((doc) => {
        ctx.builderData = (doc && typeof doc === 'object') ? doc : null;
        return ctx.builderData;
      }, () => {
        ctx.builderData = null;
        return null;
      });
    }

    // builder-art.json = where every factory preset's render puts each part
    // (crop boxes, poses, scale keys), so the builder can composite a picture
    // of a MODIFIED gun instead of showing a bare receiver. Optional in
    // exactly the same way builder.json is: null means the stage falls back
    // to the closest preset's own picture, which is still correct, just not
    // part-accurate.
    function ensureBuilderArt() {
      if (ctx.builderArt !== undefined) return Promise.resolve(ctx.builderArt);
      return loadJson('wiki/builder-art.json').then((doc) => {
        ctx.builderArt = (doc && typeof doc === 'object' && doc.items && doc.presets) ? doc : null;
        return ctx.builderArt;
      }, () => {
        ctx.builderArt = null;
        return null;
      });
    ...
    ctx.ensureBuilder = ensureBuilder;
    ctx.ensureBuilderArt = ensureBuilderArt;
    ...
    function prefetchFor(route) {
      ...
      // the weapon builder walks every slot tree (props), prices parts with
      // quest-aware offers (quests) and loads factory presets (builder)
      if (route === 'builder') return Promise.all([ensureProps(), ensureQuests(), ensureBuilder(), ensureBuilderArt()]);
