...
(function (root, factory) {
  ...
  function open(opts) {
    ...
    let lastQuestSig = null;       // JSON of the quest state the pins were last drawn from (toast gate)
    ...
    function startLive() {
      ...
      drawPings();
      if (followPlayer) snapToPlayer();
      lastQuestSig = JSON.stringify(ctx.questState || {});
      liveUnsub = lv.subscribe((ev) => {
        ...
        switch (ev.type) {
          ...
          // Refresh from Pilot (hub-pilot-connect) re-seeds quests + profile in
          // place. Redraw the quest pins and rebuild the filter panel (counts +
          // the "no quest progress" note) instead of closing the map.
          case 'quests':
          case 'profile': {
            drawStaticMarkers();
            // buildPanel() recreates the loose-loot search box empty, so leave
            // the panel alone while the player is typing in it - the pins are
            // what matters; the counts catch up on the next rebuild.
            const search = panel.querySelector('input.imap-search');
            const typing = !!search && (search.value !== '' || document.activeElement === search);
            if (!typing) buildPanel();
            if (ev.type === 'quests') {
              // The stream replays the quests frame on every (re)connect; only
              // say something when the state actually differs from last time.
              const st = ctx.questState || {};
              const sig = JSON.stringify(st);
              if (sig !== lastQuestSig) {
                lastQuestSig = sig;
                const active = Object.keys(st).filter((k) => st[k] && st[k].status === 'started').length;
                toast('Quests updated - ' + Object.keys(st).length + ' tracked, ' + active + ' active');
              }
            }
            break;
          }
    ...
    function buildPanel() {
      ...
      if (filters.questsMineOnly && !Object.keys(ctx.questState || {}).length) {
        sq.appendChild(el('div', 'fp-note',
          'no quest progress loaded - connect Fiiish Pilot above and press "Refresh from Pilot", or untick "mine only"'));
      }
