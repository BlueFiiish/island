...
(function (root, factory) {
  ...
  function applyQuests(payload) {
    ...
    // An empty map is refused on this path too (main.js never sends one, but
    // importQuestState replaces the page's state wholesale - never wipe it).
  ...
  // ---- "Refresh from Pilot" ----
  // One GET /state, applied through the same paths a live frame takes. The
  // stream already seeds quests + profile on connect, so this exists for the
  // case the stream cannot cover: the player pressed "Sync quests" in the app
  // (or the app's state moved while this tab was asleep) and wants THIS page
  // to match it now, with a sentence saying what it got.
  //
  // `quests: null` (or an empty map) in the snapshot means the Pilot has no
  // quest progress. That is NOT applied (it would wipe what this page already
  // holds - the invariant the header describes) and the readout says what to
  // do about it instead.
  let refreshing = false;
  let lastRefresh = null; // { at, ok, tasks, active, text }

  function summarizeRefresh(doc) {
    const q = doc && doc.quests;
    if (!q || typeof q !== 'object' || Array.isArray(q)) {
      return {
        ok: false, tasks: 0, active: 0,
        text: 'The Pilot has no quest progress yet - in Fiiish Pilot open Wiki & Kit > Quests > Sync quests, then refresh here.',
      };
    }
    let tasks = 0;
    let active = 0;
    Object.keys(q).forEach((id) => {
      ...
      tasks += 1;
      if (t.status === 'started') active += 1;
    });
    // An EMPTY map is the same case as null for this page: a fresh install,
    // a wrong Logs folder or a post-wipe Pilot must not erase progress that
    // was imported here by hand (TarkovTracker / file). Report it instead.
    if (tasks === 0) {
      return {
        ok: false, tasks: 0, active: 0,
        text: 'The Pilot has 0 quests on file - in Fiiish Pilot press Sync quests (Wiki & Kit > Quests) and check the readout, then refresh here.',
      };
    }
    const prof = doc.profile && typeof doc.profile === 'object' ? doc.profile : null;
    let text = 'Refreshed ' + tasks + ' quest' + (tasks === 1 ? '' : 's') + ' from the Pilot (' + active + ' active)';
    if (prof && prof.playerLevel != null) text += ' - level ' + prof.playerLevel;
    return { ok: true, tasks, active, text: text + '.' };
  }

  function refreshSnapshot() {
    if (refreshing) return Promise.resolve(lastRefresh);
    refreshing = true;
    emit('refresh', { state: 'running' });
    return fetch(BASE + '/state', { method: 'GET', credentials: 'omit' })
      .then((res) => (res.ok ? res.json() : Promise.reject(new Error('The Pilot app answered ' + res.status + '.'))))
      .then((doc) => {
        if (!doc || typeof doc !== 'object') throw new Error('The Pilot app sent an unreadable snapshot.');
        const sum = summarizeRefresh(doc);
        if (sum.ok) {
          const frame = { type: 'quests', payload: { state: doc.quests } };
          reduce(live, frame);
          applyQuests(frame.payload);
          emit('quests', frame.payload);
        }
        if (doc.profile && typeof doc.profile === 'object') {
          const frame = { type: 'profile', payload: doc.profile };
          reduce(live, frame);
          applyProfile(frame.payload);
          emit('profile', frame.payload);
        }
        if (doc.appVersion) status.appVersion = doc.appVersion;
        lastRefresh = { at: Date.now(), ok: sum.ok, tasks: sum.tasks, active: sum.active, text: sum.text };
        return lastRefresh;
      })
      .catch((e) => {
        const why = (e && e.message && e.message.indexOf('answered') !== -1)
          ? e.message
          : 'The Pilot app is not reachable at 127.0.0.1:8852 - is it running with Browser link on?';
        lastRefresh = { at: Date.now(), ok: false, tasks: 0, active: 0, text: why, error: true };
        return lastRefresh;
      })
      .then((r) => {
        refreshing = false;
        emit('refresh', { state: 'done', result: r });
        return r;
      });
  }
  ...
  function mountUi() {
    ...
    const refreshBtn = document.getElementById('tk-pilot-refresh');
    ...
    function render() {
      ...
      if (stateEl) {
        ...
        stateEl.classList.toggle('on', linked);
      ...
      if (refreshBtn) {
        // Only offered while linked: a refresh with no app to ask is the
        // connect button's job, and its own error line already says so.
        refreshBtn.hidden = !linked;
        refreshBtn.disabled = refreshing;
        refreshBtn.textContent = refreshing ? 'Refreshing...' : 'Refresh from Pilot';
      }
      if (hintEl) {
        // The hint line is shared: a link error wins, otherwise the last
        // refresh's readout stays up so "did that work?" has an answer.
        if (st.state === 'error') {
          hintEl.hidden = false;
          hintEl.textContent = st.error || '';
          hintEl.classList.add('bad');
        } else if (lastRefresh) {
          hintEl.hidden = false;
          hintEl.textContent = lastRefresh.text || '';
          hintEl.classList.toggle('bad', !lastRefresh.ok);
        } else {
          hintEl.hidden = true;
    ...
    if (refreshBtn) refreshBtn.addEventListener('click', () => { refreshSnapshot().then(render); render(); });
    subscribe((ev) => {
      if (ev.type === 'status' || ev.type === 'hello' || ev.type === 'map' || ev.type === 'refresh') render();
    });
    render();
    return render;
  }
  ...
  return {
    ...
    refreshSnapshot, summarizeRefresh,
