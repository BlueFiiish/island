...
// VENDORED, PINNED, SAME-ORIGIN (2026-09-09, client-audit C-02).
// This used to be `cdn.jsdelivr.net/npm/tesseract.js@5/...` - a floating major
// with no SRI, injected into document.head with full same-origin privileges on
// an origin that stores bearer tokens. tesseract.js then pulled its worker, its
// WASM core and its language data from two more CDNs at runtime. The whole
// engine now lives in ./vendor/tesseract/ at an exact pin; see its VENDORED.md.
// Nothing here is loaded until the user actually clicks "scan".
const TESS = {
  lib: '/island/apps/rust/vendor/tesseract/tesseract.min.js',
  workerPath: '/island/apps/rust/vendor/tesseract/worker.min.js',
  corePath: '/island/apps/rust/vendor/tesseract/core',
  langPath: '/island/apps/rust/vendor/tesseract/lang',
};
...
async function scanStart() {
  ...
  try {
    ...
    if (!GENE.worker) {
      // Every path is local (C-02). `workerBlobURL: false` makes the Web Worker
      // a plain same-origin Worker instead of a blob: URL wrapper, so the CSP
      // needs `worker-src 'self'` and nothing else.
      GENE.worker = await Tesseract.createWorker('eng', 1, {
